...
if [[ `uname` == 'Linux' ]]; then
  current_charge=$(cat /proc/acpi/battery/CMB1/state | grep 'remaining capacity' | awk '{print $3}')
  total_charge=$(cat /proc/acpi/battery/CMB1/info | grep 'last full capacity' | awk '{print $4}')
else
